; Three rolling rows, a nine-bit sliding neighborhood, and a 64-byte rule table.
...
%define SPARKS 6
%endif
%if SPARKS < 0 || SPARKS > 65535
%error SPARKS must fit an unsigned 16-bit count
%endif
%ifndef GLIDERS
%define GLIDERS 1
%endif
...
.live:
    ...
    add al, 44
    call pout
    ; Live BL is 128..255: (BL >> 1) - 48 equals ((BL & 127) >> 1) + 16.
    mov al, bl
    shr al, 1
    sub al, 48
    call pout
...
.loaded:
    ...
    mov si, 323
    mov cx, 320
    ; Bit 1 means dead. Start with the missing left column and current column.
    mov bx, 7
    ...
    rcl bx, 1
.cell:
    ; Shift out the old left column and shift in the new right column.
    ...
    and bx, 511
    mov al, [si]
    ; FS is zero, so the ORG-based table address is independent of BIOS CS.
    bt word [fs:rule_bits], bx
    jnc .off
.on:
    or al, al
    js .age
...
rnd:
    ...
    mov ax, bp
    ret
; NASM constructs 512 rule bits, packed into 32 little-endian words.
; Index: left top/mid/bottom in bits 8..6, center in 5..3, right in 2..0.
rule_bits:
%assign pattern 0
%rep 32
    %assign packed 0
    ...
    %rep 16
        ...
        %assign shift 0
        %rep 9
            %assign dead dead + ((pattern >> shift) & 1)
            %assign shift shift + 1
        %endrep
        %assign alive (dead = 6) || ((dead = 5) && ((pattern & 16) = 0))
        %assign packed packed | (alive << bit)
